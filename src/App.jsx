  ...
  return (
    <div className="page-shell">
      ...
      <header className="site-header">
        <div className="container header-inner">
          <a className="brand" href="#home" aria-label="Bluvera Exports home">
            <img src="/logo.jpg" alt="Bluvera Exports logo" className="logo" fetchPriority="high" decoding="async" />
          </a>
          <button
      ...
      <main>
        <section id="home" className={`hero container ${visible.has('hero') ? 'is-visible' : ''}`} data-reveal="hero">
          ...
          <div className="hero-visual" ref={heroRef}>
            ...
            <img src="/hero.jpg" alt="Premium Ayurvedic raw materials exporter from India" className="hero-image" fetchPriority="high" decoding="async" />
          ...
          <div className="cats-grid">
            {categories.map((item, index) => (
              <a className="cat-card image-card" href="#enquiry" key={item.name} style={{ animationDelay: `${index * 90}ms` }}>
                <img src={item.image} alt={`${item.name} export category`} className="cat-image" loading="lazy" decoding="async" />
                <span>{item.name}</span>
              </a>
            ))}
          </div>
        ...
        <section id="about" className={`section container split ${visible.has('about') ? 'is-visible' : ''}`} data-reveal="about">
          <div className="split-media">
            <img src="/farm.jpg" alt="Herbal sourcing farm in India" loading="lazy" decoding="async" />
          </div>
            ...
            {filteredProduct ? (
              <article className="product-card hover-rise">
                <div className="product-image-shell">
                  <img src={filteredProduct.image} alt="Ashwagandha raw material by Bluvera Exports" loading="lazy" decoding="async" />
                </div>
          ...
          <div className="cert-grid">
            {certifications.map((cert) => (
              <article className="cert-card cert-card-image" key={cert.name}>
                <img src={cert.image} alt={`${cert.name} certification badge`} loading="lazy" decoding="async" />
                <h3>{cert.name}</h3>
                <p>{cert.detail}</p>
              </article>
            ))}
      ...
      <footer className="site-footer">
        <div className="container footer-inner">
          <div className="footer-about">
            <img src="/logo.jpg" alt="Bluvera Exports footer logo" className="logo" loading="lazy" decoding="async" />
      ...
      <a className="float-quote" href="#enquiry" aria-label="Request Quote">
        <svg className="cta-icon cta-icon-message" viewBox="0 0 24 24" aria-hidden="true">
          <path d="M4 4.75A2.75 2.75 0 0 1 6.75 2h10.5A2.75 2.75 0 0 1 20 4.75v8.5A2.75 2.75 0 0 1 17.25 16h-6.44L6.7 19.34A1.25 1.25 0 0 1 4.5 18.47V16h-1.75A2.75 2.75 0 0 1 0 13.25v-8.5A2.75 2.75 0 0 1 2.75 2H4v2.75Zm2.75-1.25A1.25 1.25 0 0 0 5.5 4.75v8.5c0 .69.56 1.25 1.25 1.25H7v2.46L10.83 14.5h6.42a1.25 1.25 0 0 0 1.25-1.25v-8.5a1.25 1.25 0 0 0-1.25-1.25H6.75Z" />
        </svg>
        <span className="cta-label">Get Quote</span>
      </a>
      <a className="wa-stick" href={whatsappLink} target="_blank" rel="noreferrer" aria-label="Chat on WhatsApp">
        <svg className="cta-icon cta-icon-whatsapp" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a9.9 9.9 0 0 0-8.52 14.86L2 22l5.32-1.4A10 10 0 1 0 12 2Zm0 18.1a8.1 8.1 0 0 1-4.13-1.13l-.3-.17-3.16.83.85-3.07-.2-.32A8.1 8.1 0 1 1 12 20.1Zm4.7-5.25c-.25-.12-1.48-.73-1.7-.81-.23-.09-.4-.12-.57.12s-.66.81-.81.98-.3.18-.55.06a6.6 6.6 0 0 1-1.95-1.2 7.3 7.3 0 0 1-1.35-1.67c-.14-.24-.01-.37.11-.49.11-.11.24-.3.36-.45.12-.15.16-.25.25-.42.08-.17.04-.31-.02-.43-.06-.12-.57-1.37-.78-1.88-.2-.48-.41-.41-.57-.42h-.48c-.16 0-.43.06-.66.31s-.88.86-.88 2.1.9 2.43 1.02 2.6c.13.17 1.74 2.66 4.2 3.72.58.25 1.03.4 1.38.51.58.18 1.1.16 1.5.1.46-.07 1.48-.6 1.69-1.18.21-.57.21-1.06.15-1.18-.06-.12-.22-.18-.47-.3Z"/></svg>
